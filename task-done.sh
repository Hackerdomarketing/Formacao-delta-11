...
echo ""
echo -e "  ${BOLD}3.4${NC} [ ] ${RED}AUTOCRÍTICA EM 2 PARADAS REGISTRADA?${NC} (v5.3)"
echo -e "       → Parada 1 (após escrever, antes de validar): ≥3 bugs prováveis CONCRETOS + ≥3 casos extremos"
echo -e "       → Parada 2 (após testes passarem): casos extremos viraram teste? hardcoded? console.log? limites §8?"
echo -e "       → SALVE em: ${BOLD}.delta-11/logs/autocritica/$(date +%Y-%m-%d)-$TASK_ID-$AGENTE.md${NC}"
echo -e "       → ${RED}SHIELD devolve tarefa sem revisar se este arquivo estiver ausente ou genérico.${NC}"
echo ""
...
echo ""
echo -e "  ${BOLD}3.7${NC} [ ] ${YELLOW}CONTRACT-TESTER (antes de mandar para o SHIELD):${NC}"
echo -e "       → USE Task tool com subagent_type: \"general-purpose\""
echo -e "       → PROMPT: conteúdo de .delta-11/sub-agentes/contract-tester.md + \"Agente: $AGENTE. Arquivos: <lista>\""
echo -e "       → Desvio de contrato = corrigir antes de enviar ao SHIELD"
echo ""
echo -e "  ${BOLD}3.8${NC} [ ] ${YELLOW}REVISÃO DO SHIELD:${NC}"
echo -e "       → Prompt pronto em: ${BOLD}$SHIELD_FILE${NC}"
echo -e "       → SendMessage ao CRONOS (v4.0 Onda 2) — ele dispara o SHIELD via Agent tool"
echo ""
echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
echo ""
